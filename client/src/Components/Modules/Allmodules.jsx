...
const Allmodules = ({ progress }) => {
	...
	const filteredModules = allModules.filter(
		(module) => !modules.includes(module.unique)
	);
		...
		<div>
			{filteredModules.length <= 0 && (
				<div className='w-full text-center mt-5'>
					<p>
						No modules available. We will notify you when more modules are
						created.
					</p>
				</div>
			)}
			<div className='grid lg:grid-cols-2 gap-3 mt-5 mb-36'>
				{filteredModules.map((lang) => {
					const limitedDescription = lang.description
						.split(' ')
						.slice(0, 20)
						.join(' ');
					return (
						<div
							key={lang.id}
							className='w-full max-w-lg bg-blue-500/5 border border-blue-500/15 rounded-2xl hover:bg-blue-500/10 hover:border-blue-500/20 duration-200'>
