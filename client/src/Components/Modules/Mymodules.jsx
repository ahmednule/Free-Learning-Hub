...
const Mymodules = ({ progress }) => {
	...
	let htmlCssProgress = progress['html-css']
		? progress['html-css'].progress
		: 0;
	...
	const filteredModules = allModules.filter((module) =>
		modules.includes(module.unique)
	);
		...
		<div>
			{modules.length <= 0 && (
				<div className='w-full text-center mt-5'>
					<p>
						You don&apos;t have any registered module. Switch to{' '}
						<span className='code'>all modules</span> to register a module and
						start learning.
					</p>
				</div>
			)}
				...
				{filteredModules.map((lang) => {
					...
					if (lang.unique === 'html-css') {
						progressPercent = Math.round(
							(htmlCssProgress / userDataMain.htmlCss) * 100
						);
					} else if (lang.unique === 'python') {
						progressPercent = Math.round(
							(pythonProgress / userDataMain.python) * 100
						);
					}
					...
					return (
						<div
							key={lang.id}
							className='w-full max-w-sm bg-blue-500/5 border p-4 border-blue-500/15 rounded-2xl hover:bg-blue-500/10 hover:border-blue-500/20 duration-200'>
