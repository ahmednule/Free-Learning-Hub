...
const Dashboard = () => {
	...
	const modules = Object.keys(userDataMain?.userProgress?.progress).length | 0;
	const cubesPercentage = Math.floor(
		(userDataMain.userCubes / userDataMain.cubes) * 100
	);
		...
		<div className='w-full bg-primary-700 px-2 py-5 rounded-sm'>
			<div className='flex flex-col md:flex-row flex-wrap gap-8'>
				<div className='bg-primary-600 rounded-md py-4 px-7'>
					<h2 className='text-2xl md:text-3xl text-center sm:text-left'>
						Your progress
					</h2>
					<div className='flex flex-col sm:flex-row items-center gap-10'>
						...
						<div>
							<p className='md:text-2xl font-light mb-4 text-reds-400'>
								FLH Cubes
							</p>
							<p className='text-sm md:text-base'>
								{userDataMain.userCubes} cubes
							</p>
						</div>
					</div>
				</div>
				...
				<div className='w-[340px] py-4 px-6 bg-primary-600 rounded-md grid grid-cols-2 gap-5'>
					{noName.map((item, index) => {
						return (
							<div
								key={index}
								className='h-[90px] w-[140px] bg-primary-400/30 border cursor-pointer border-primary-300/50 hover:scale-105 hover:bg-primary-700 duration-200 rounded'>
