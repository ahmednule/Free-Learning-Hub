...
import { createInProgress, createIsCompleted, createAllModules } from './Modules.help';
            ...
            inProgress.map((item, index) => {
              return (
                ...
                >
                  ...
                  <Link to={item.link}>
                    <p className='bg-yellow-500 text-gray-950 hover:bg-yellow-600 duration-200 rounded py-1 px-4'>RESUME</p>
                  </Link>
                </div>
              )
            })
            ...
            isComplete.map((item, index) => {
              return (
                ...
                >
                  ...
                  <Link to={item.link}>
                    <p className='bg-green-500 text-gray-950 cursor-pointer hover:bg-green-600 duration-200 rounded py-1 px-4'>CERTIFICATE</p>
                  </Link>
                </div>
              )
            })
            ...
            allModules.map((item, index) => {
              return (
                ...
                >
                  <p className='mt-1'>{index + 1}. {item.name}</p>
                  <p className='mt-1'>
                    {item.progress === 100 ? 'COMPLETE' : `${item.progress}% done` }
                  </p>
                  <Link to={item.link}>
                    <p
                      className={`text-gray-950 cursor-pointer hover:bg-opacity-80 duration-200 rounded py-1 px-4 ${
                        item.type === 'progress' ? 'bg-yellow-500 hover:bg-yellow-600' :
                        item.type === 'done' ? 'bg-green-500 hover:bg-green-600' :
                        item.type === 'register' ? 'bg-blue-500 hover:bg-blue-600' : ''
                      }`}
                    >
